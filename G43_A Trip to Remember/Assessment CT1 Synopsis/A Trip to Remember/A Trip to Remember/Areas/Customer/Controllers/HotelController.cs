...
namespace ATripToRemember.Areas.Customer.Controllers
{
    ...
    public class HotelController : Controller
    {
        ...
        public IActionResult Index(string? city, string? state)
        {
            ...
            {
                hotelList = unitOfWork.Hotel.GetAll(u => u.State == state).ToList();
            }
            else if (state == null)
            {
                hotelList = unitOfWork.Hotel.GetAll(u => u.City == city).ToList();
            }
            else
            {
                hotelList = unitOfWork.Hotel.GetAll(u => u.City == city && u.State == state).ToList();
            }
            if(hotelList.Count() == 0)
            {
                TempData["error"] = "No Hotel Found";
                return View(hotelList);
            }
            return View(hotelList);
        }
