...
namespace ATripToRemember.Areas.Customer.Controllers
{
    ...
    public class FlightController : Controller
    {
        ...
        [HttpPost]
        public IActionResult Index(string? fromCity, string? toCity)
        {
            IEnumerable<Flight> flightList;
            if (fromCity != null && toCity != null)
            {
                flightList = unitOfWork.Flight.GetAll(u => u.StartingLocation == fromCity && u.Destination == toCity, includeProperties: "Airline").ToList(); ;
            }
            else
            {
                TempData["error"] = "Fill both fields for searching a flight.";
                flightList = unitOfWork.Flight.GetAll(includeProperties: "Airline").ToList();
                return View(flightList);
            }
            if (flightList.Count() == 0)
            {
                TempData["error"] = "No Flight Found";
            }
            return View(flightList);
        }
